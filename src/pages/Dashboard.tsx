import { 
  Calendar, 
  ListChecks, 
  CheckSquare, 
  AlertTriangle,
  TrendingUp 
} from 'lucide-react';
import { AppLayout } from '@/components/layout/AppLayout';
import { MetricCard } from '@/components/dashboard/MetricCard';
import { UpcomingMeetings } from '@/components/dashboard/UpcomingMeetings';
import { AgendaPipeline } from '@/components/dashboard/AgendaPipeline';
import { RecentActions } from '@/components/dashboard/RecentActions';
import { dashboardMetrics, agendaPoints, actions } from '@/data/mockData';

export default function Dashboard() {
  return (
    <AppLayout 
      title="Dashboard" 
      subtitle="Visão geral das atividades do Board"
    >
      <div className="space-y-6">
        {/* Metrics Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            title="Reuniões Agendadas"
            value={dashboardMetrics.upcomingMeetings.length}
            subtitle="Próximos 30 dias"
            icon={<Calendar className="w-6 h-6 text-accent" />}
            variant="accent"
          />
          <MetricCard
            title="Pontos Pendentes"
            value={dashboardMetrics.pendingAgendaPoints}
            subtitle="Aguardam decisão"
            icon={<ListChecks className="w-6 h-6 text-state-proposed" />}
            variant="warning"
          />
          <MetricCard
            title="Ações Ativas"
            value={dashboardMetrics.activeActions}
            subtitle={`${dashboardMetrics.overdueActions} em atraso`}
            icon={<CheckSquare className="w-6 h-6 text-status-info" />}
            trend={dashboardMetrics.overdueActions > 0 ? { 
              value: dashboardMetrics.overdueActions, 
              isPositive: false 
            } : undefined}
          />
          <MetricCard
            title="Taxa de Conclusão"
            value={`${dashboardMetrics.completionRate}%`}
            subtitle="Ações concluídas"
            icon={<TrendingUp className="w-6 h-6 text-status-success" />}
            variant="success"
            trend={{ value: 8, isPositive: true }}
          />
        </div>

        {/* Main Content Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left Column - Takes 2/3 on large screens */}
          <div className="lg:col-span-2 space-y-6">
            <AgendaPipeline points={agendaPoints} />
            <RecentActions actions={actions} />
          </div>

          {/* Right Column - Takes 1/3 on large screens */}
          <div className="space-y-6">
            <UpcomingMeetings meetings={dashboardMetrics.upcomingMeetings} />
            
            {/* Quick Stats Card */}
            <div className="bg-card rounded-xl border border-border/50 shadow-card p-6">
              <h3 className="font-semibold text-foreground mb-4">Resumo Mensal</h3>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Decisões tomadas</span>
                  <span className="text-sm font-semibold text-foreground">12</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Ações criadas</span>
                  <span className="text-sm font-semibold text-foreground">28</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Ações concluídas</span>
                  <span className="text-sm font-semibold text-foreground">15</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Reuniões realizadas</span>
                  <span className="text-sm font-semibold text-foreground">4</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </AppLayout>
  );
}
